...
#Busqueda de clientes, esperemos q si funcione
class ResPartner(models.Model):
    ...
    def nameSearch(self):
        ...
        #Normaliza los dígitos de los telefonos
        digits = re.sub(r'\D', '', name)

        #Prioriza coincidencias exactas de RFC o email
        exact_domain = ['|', ('vat', '=', name), ('email', '=', name)]
        exact_domain = ['&', ('customer_rank', '>', 0)] + exact_domain
        partners = self.search(exact_domain + args, limit=limit)
        if not partners:
            # Búsqueda amplia (OR) por varios campos
            wide = ['|', '|', '|', '|', '|',
                    ('name', operator, name),
                    ('vat', operator, name),
                    ('email', operator, name),
            ...
            if digits:
                # también por números de teléfono “sin formato”
                wide = ['|', ('phone', 'ilike', digits), '|', ('mobile', 'ilike', digits)] + wide
            partners = self.search(['&', ('customer_rank', '>', 0)] + wide + args, limit=limit)
